 ...
 */
package org.mausam.opticals.cockpits.setup;
...
import org.mausam.opticals.cockpits.constants.OpticalsCockpitsConstants;
 ...
 */
@SystemSetup(extension = OpticalsCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(OpticalsCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(OpticalsCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						OpticalsCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/opticalscockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
